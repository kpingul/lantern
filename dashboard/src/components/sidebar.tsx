...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 h-screen w-64 border-r border-[rgb(var(--border-subtle))] bg-[rgb(var(--bg-secondary))] flex flex-col">
      {/* Logo */}
      <div className="flex h-16 items-center gap-3 border-b border-[rgb(var(--border-subtle))] px-6">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500 to-cyan-600 shadow-lg shadow-cyan-500/30">
      ...
      <nav className="flex flex-col gap-1 p-4 flex-1">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`transition-colors ${isActive ? 'text-cyan-600' : 'text-[rgb(var(--text-muted))] group-hover:text-cyan-600'}`}>
                {item.icon}
              </span>
              {item.label}
            </Link>
